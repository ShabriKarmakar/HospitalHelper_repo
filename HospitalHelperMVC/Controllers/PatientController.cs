    ...
    public class PatientController : Controller
    {
...

        {
            ...
            else
            {
                using (HospHelperEntities db = new HospHelperEntities())
                {
                    return View(db.PatientTables.Where(x => x.PatientID == id).FirstOrDefault<PatientTable>());
                }
            }
        ...
        [HttpPost]
        public ActionResult AddOrEdit(PatientTable pat)
        {
            using (HospHelperEntities db = new HospHelperEntities())
            {
        ...
        public ActionResult Delete(int id)
        {
            using (HospHelperEntities db = new HospHelperEntities())
            {
                PatientTable pat = db.PatientTables.Where(x => x.PatientID == id).FirstOrDefault<PatientTable>();
                db.PatientTables.Remove(pat);
                db.SaveChanges();
